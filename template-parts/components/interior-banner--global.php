<?php 
  if ( have_rows( 'custom_interior_banner' , 'themes-options' ) ) :
    while ( have_rows( 'custom_interior_banner' , 'themes-options' ) ) : the_row();
...
<header class="padding-y-xl interior-banner bg-<?= $background_color; ?>" >
      ...
      if (is_post_type_archive( 'services' )) {
        if (have_rows( 'service_introduction', 'service-options' )) {
          while (have_rows( 'service_introduction', 'service-options' )) {
            the_row();
            echo '<div class="text-center text-component">';
            the_sub_field( 'introduction' );
            echo '</div>';
          }
        } 
      } elseif ( is_post_type_archive( 'products' )) {
        if (have_rows( 'product_introduction', 'product-options' )) {
          while (have_rows( 'product_introduction', 'product-options' )) {
            the_row();
            echo '<div class="text-center text-component">';
            the_sub_field( 'introduction' );
            echo '</div>';
        ...
        if (have_rows( 'resource_introduction', 'resource-options' )) {
          while (have_rows( 'resource_introduction', 'resource-options' )) {
            the_row();
            echo '<div class="text-center text-component">';
            the_sub_field( 'introduction' );
            echo '</div>';
