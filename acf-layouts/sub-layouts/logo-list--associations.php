...
$logos = $this->row['association_list'];
...
if($logoCount <= 4) {
  echo '<ul class="list-inline logo-list logo-list--manufacturers justify-center">';
    ...
    foreach ($logos as $logo) {
      ...
      if($white_logos == true) {
        if($size == 'standard') {
          echo '<img src="'.$assiociations_list[$item]['white-logo'].'" alt="'.$item .' logo" class="logo-list__logo--standard">';
        } else {
          echo '<img src="'.$assiociations_list[$item]['white-logo'].'" alt="'.$item .' logo" class="logo-list__logo--small">';
        } 
      } else {
        if($size == 'standard') {
          echo '<img src="'.$assiociations_list[$item]['color-logo'].'" alt="'.$item .' logo" class="logo-list__logo--standard '.$greyImg.'">';
        } else {
          echo '<img src="'.$assiociations_list[$item]['color-logo'].'" alt="'.$item .' logo" class="logo-list__logo--small '.$greyImg.'">';
        } 
      }
...
if($logoCount > 4) {
  ...
  if(isset($title)) {
    ...
    echo '<div class="col-12"><div class="swiper-container logo-list__carousel logo-list__carousel--associations "><div class="swiper-wrapper logo-list__wrapper">';
    foreach ($logos as $logo) {
      $item = $logo['value'];
    ...
    // echo '<div class="col-12 text-center"><a id="view-all--associations" class="btn btn--gray btn--sm"><i class="fas fa-th"></i> View All</a></div>';
    echo '<div class="navigation-inline margin-top-lg"><div class="swiper-navigation"><div class="swiper-button-prev swiper-button-prev--associations"></div><div class="swiper-pagination swiper-pagination--associations swiper-pagination--logo-list"></div><div class="swiper-button-next swiper-button-next--associations"></div></div></div>';
  }
}