...
if($singleLayoutStyle == 'full-width') {
  ...
  if($suite) {
    $destination = $address . ',<br/>' . $suite . ',<br />' . $city . ', ' . $state . ' ' . $zip;
  } else {
    $destination = $address . ',<br/>' . $city . ',<br/>' . $state . $zip;
  }
  ...
  echo '<div class="col-8@md col-12 '.$side_left.'"><div class="split-content">' . do_shortcode( '[wpsl_map id="'.$locationId.'" zoom="18"]') . '</div></div>'; 
  ...
  echo '<p><a href="'.get_the_permalink($locationId).'" class="btn btn--primary btn--sm">Contact Clinic</a></p>';
  echo '</div>';
